...
import { Table, Row, Col, Card, Modal, Divider, notification, Popconfirm, AutoComplete, Input, Icon, Dropdown, Menu, Button, Select } from 'antd';
...
class InvkeGrid extends React.Component {
    ...
    columns = [
        ...
        {
            dataIndex: 'systemId', title: '调用系统', width: 100,
            render: (text, record) => {
                if (this.state.systemInfo.length == 0) {
                    return '';
                }
                return this.state.systemInfo.find(s => s.systemId == text).name
            }
        },
        {
    ...
    loadData = async (page, start, limit, invokeName = this.state.selectInvokeName, groupName = this.state.selectgroupname) => {
        ...
        console.log()
        let json = await post(url, { start: start, limit: limit, invokeName: invokeName, groupName, systemId: this.state.selectSystem });
        if (json) {
        ...
        return (
            <div>
                <Row gutter={2} className="table-head-row">
                    ...
                    <Col span={2} className="col-label">调用系统:</Col>
                    <Col span={4}>
                        <Select onChange={this.selectSystem} style={{ width: '100%' }} allowClear={true} >
                            {
                                this.state.systemInfo.map((o, i) => {
